...
cat  >/etc/openvpn/server-ohp.conf <<-EOFOHP
port 2834
proto tcp
dev tun
ca /etc/openvpn/pki/ca.crt
...
cat  >/etc/openvpn/server-ws.conf <<-EOFWS
port 2913
proto tcp
dev tun
ca /etc/openvpn/pki/ca.crt
...
remote-cert-tls server
cipher AES-256-GCM
auth-user-pass" >/etc/openvpn/client/openvpn-ws.ovpn

{
    ...
    cat /etc/openvpn/pki/ca.crt
    echo "</ca>"
} >>/etc/openvpn/client/openvpn-ws.ovpn
cp /etc/openvpn/client/openvpn-ws.ovpn /usr/share/nginx/html/openvpn-ws.ovpn

systemctl enable openvpn@server-tcp
systemctl enable openvpn@server-tls
systemctl enable openvpn@server-ohp
systemctl enable openvpn@server-ws
