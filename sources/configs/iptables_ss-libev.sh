...
# which is handy if you use an OpenWRT-powered router.
###
...
#
# Executing this script on the linux host can proxy all outgoing traffic of this machine (except the traffic sent to the reserved address).
# Other hosts under the same LAN can also change their default gateway to the ip of this linux host 
# (at the same time change the dns server to 1.1.1.1 or 8.8.8.8, etc.) to proxy their outgoing traffic.
#
