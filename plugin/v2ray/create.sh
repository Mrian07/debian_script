...
function trojan {
    ...
    jq '.inbounds[0].settings.clients += [{"password": "'"${UUID}"'","email": "'"${getUser}"@"${DOMAIN}"'"}]' /usr/local/etc/v2ray/trojan-tcp-tls.json
...
function vmess {
    ...
    while true; do
        ...
        03)
            ...
            cat /usr/local/etc/v2ray/vmess-tls-ws.json | jq '.inbounds[0].settings.clients += [{"id": "'${UUID}'"}]' >/usr/local/etc/v2ray/vmess-tls-ws_tmp.json
            mv -f /usr/local/etc/v2ray/vmess-tls-ws_tmp.json /usr/local/etc/v2ray/vmess-tls-ws.json
            systemctl restart v2ray@vmess-tls-ws
